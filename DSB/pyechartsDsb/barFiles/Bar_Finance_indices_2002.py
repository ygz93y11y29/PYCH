...
def get_year_overlap_chart(year: int) -> Bar:
    bar = (
        Bar()
        #新增 X 轴数据   #xaxis_data：X 轴数据项 或直接【】
        ...
        .set_global_opts(
            ...
            #tooltip_opts：提示框配置     TooltipOpts：提示框配置项  is_show:是否显示提示框组件，包括提示框浮层和 axisPointer。     trigger:触发类型  axis_pointer_type:指示器类型'shadow'：阴影指示器
    ...
    pie = (
        ...
        .add(
            ...
            radius="28%", #等同于radius=["","28%"],
        )
        # set_series_opts：系统配置项        tooltip_opts：工具配置      TooltipOpts：提示框配置项  is_show：是否显示提示框组件，包括提示框浮层和 axisPointer。 trigger：触发类型。
...
#      is_auto_play:自动播放，play_interval：播放间隔
